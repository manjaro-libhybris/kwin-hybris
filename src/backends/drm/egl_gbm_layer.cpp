...
{
    // take care of dmabuf feedback
    if (m_oldScanoutCandidate && m_oldScanoutCandidate->dmabufFeedbackV1()) {
        m_oldScanoutCandidate->dmabufFeedbackV1()->setTranches({});
    }
    m_oldScanoutCandidate = nullptr;
...
bool EglGbmLayer::createGbmSurface()
{
    ...
    const bool allowModifiers = (m_renderGpu->isNVidia() && !modifiersEnvSet) || (modifiersEnvSet && modifiersEnv);
...
bool EglGbmLayer::doesGbmSurfaceFit(GbmSurface *surf) const
{
    return surf && surf->size() == m_output->bufferSize()
        ...
        && m_output->supportedModifiers(surf->format()) == surf->modifiers();
}
...
bool EglGbmLayer::doesShadowBufferFit(ShadowBuffer *buffer) const
{
    if (m_output->needsSoftwareTransformation()) {
        ...
        // TODO check the format as well
        return buffer && buffer->textureSize() == m_output->sourceSize();
    }
}
